...
with SB(uc=True, binary_location=binary_location, headless=True) as sb:
    ...
    try:
        ...
        sb.type("#inputEmail", username)
        sb.type("#inputPassword", password)

        # username_input.send_keys(username)
        # password_input.send_keys(password)

        sb.click("div.checkbox-styled")
        log_output(f"✅ checkbox 已勾选")
        ...
        if "Connection refused" in str(e):
            log_output(f"❌ 连接被拒绝，请检查网络连接 {e}")
            # sb.quit()
            exit()
        log_output(f"登录失败或无需登录 {e}")
